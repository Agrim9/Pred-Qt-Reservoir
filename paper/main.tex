...
Past methods have primarily exploited the nonlinear geometry endowed by the manifold structure \textcolor{red}{\sout{to}} for precoder prediction.
...
Given the high dimensionality of the precoders (which form a $N_T \times N_R$ complex\textcolor{red}{-}valued matrix, viz. $2\times N_T\times N_R$ real numbers), effective quantization with just a few bits poses a significant challenge.
...
Its design is inspired \textcolor{red}{by} several frameworks of recurrent neural networks \cite{jaeger2004harnessing}, including Liquid State Machines \cite{jalalvand2015real}.
A reservoir computing system maps the inputs to \textcolor{red}{spatiotemporal} patterns in a higher dimensional dynamical reservoir state.
The reservior states are then lineraly combined to create the output. The only trainable parameters of the system are those of the output coupler. These parameters can be learned by using regression.
...
The key advantage of a reservoir computing based system is that it is extremely easy to train in comparison to its counterparts like Deep Neural Networks \cite{Schrauwen07anoverview}.
The input features can be efficiently \textcolor{red}{learned} using a simple learning algorithm since the reservoir non-linearly transforms sequential inputs into a higher dimensional reservoir state.
