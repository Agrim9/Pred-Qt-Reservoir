...
In MIMO wireless systems, precoding matrices are used for transformation of an $N_S$-dimensional ($N_S \leq \text{min}(N_T,N_R))$ data stream onto an $N_T$-dimensional transmit vector that corresponds to the signal emanating from the $N_T$ transmit antennas.
...
First, the optimum precoders are not actually $2N_TN_R$ dimensional entities, but have an underlying manifold structure that allows to work with lower dimensions, when performing operations over the manifold directly \cite{pitaval2013codebooks}.
