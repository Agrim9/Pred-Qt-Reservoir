...
However, these approaches are difficult to implement and require some operations on the manifold structure.
...
Finally, Section \ref{section5} concludes and presents future lines of work.
...
Depending on whether $\text{fb}_{i,t} \geq 2^{\mathcal{B}-1}$, i.e. whether the optimum codeword is in $T^{\mathcal{C}}_p$ or $T^{\mathcal{C}}_m$, the scale parameter $s[k]$, which in turn controls values of $s_p, s_m$, is updated accordingly, as
...
\end{equation}
Computing the gradient $\frac{\partial \mathcal{O}_{i,t}}{\partial \bW_{i,t}^{\text{out}}}$ and setting it to the null matrix yields
\begin{equation}
...
Finally, we summarize and highlight the novel aspects of the proposed framework for predictive quantization.
The first key advantage of the proposed approach over the past work \cite{Gupt1905:Predictive,6891198,6545375} is that the prediction function and quantization function are not independent of each other.
...
\label{pred}
\end{equation}
The dependence between $f_P$ and $q_P$ in our framework is in the sense that the backward pass ensures that prediction function `learns' the correlations brought about by $q_P$, in addition to temporal correlations. %Better framing possible
...
\textcolor{blue}{Another advantage of the proposed scheme is the ease of training the reservoir, captured in \eqref{res_upd}.
...
This is a departure from previous schemes \cite{Gupt1905:Predictive,6891198,6545375} which store the past $n$ precoders precoders in a $n$ sized cyclic buffer ($n$ also has to be pre-decided).
...
\label{setting}
\noindent We simulate both our predictive quantization algorithm and the one presented in \cite{6891198} for the IEEE Pedestrian-A channel, with $N_T=4$, $N_R=2$. 
The channel matrices $\bH_{i,t}$ are generated using Jake's model for realistic simulations, from the IT++ library through the python wrapper py-itpp \cite{ViditPy}. 
Although in both our work and \cite{6891198}, only temporal correlations are exploited, we compare the BER, quantization error and logsum rate results for the complete MIMO-OFDM setting with $N=64$ of subcarriers, and channels are fed back for $8$ evenly spaced subcarriers, indexed starting at 0, viz. $\{0,9,\ldots,63\}$ ($\{9k, k \in \{0,1,\ldots,7\}\}$).
The precoding matrices at subcarriers were channels are not fed back ($i\neq\{9k\}$) are estimated via the Cayley Interpolation method presented in \cite{Gupt1905:Predictive}.
Feedback is available for $i=\{9k\}$ subcarriers (i.e. transmitter obtains $\text{fb}_{\{9k\},t}$) for predictive quantization.
The setting is visually illustrated in Fig. \ref{table}.
A 6 bit codebook generated using the Lloyd codebook algorithm for the Stiefel Manifold in \cite{6678348} is used to feedback $\tilde{\bU}_{\{9k\},0}$ and differential quantization (i.e. $\bP_{i,t}=\tilde{\bU}_{i,t-1}$) is used for 10 time instances to provide provide the initial training data for the reservoir and for overcoming initial transient response of the reservoir, as discussed in \cite{mosleh2017brain}.
A 6 bit base codebook that is used for the quantization function $q_P$ is generated according to the method presented in Section \ref{qp} and \cite{Gupt1905:Predictive}.
Hence, for both initialization and subsequent feedback, $\text{fb}_{i,t}$ can be encoded using 6 bits (i.e. $\mathcal{B}=6$ Section \ref{section2}).
...
\label{res}{}
To mitigate errors in tracking in the presence of randomly appearing drastic events in wireless channels, such as sudden interferences and dynamic environments, only 100 channel evolutions are considered for the simulations.
Averaging is performed over 10 independent channel realizations, to ensure that the simulation results hold in general for the chosen IEEE Pedestrian A channel profile. Therefore, the quantization error, BER and achievable rate results are averaged over 1000 channel instances, with 100 channel evolutions of 10 independent channel realizations.
\begin{figure}[h]
\centering
\includegraphics[width=0.45\textwidth]{images/qtizErr.pdf}
\caption{Quantization error in terms of chordal distance to the actual precoder $\bU_{\{9k\},t}$ observed at the receiver, from the quantized estimate $\tilde{\bU}_{\{9k\},t}$ at the transmitter, averaged over 8 fed back subcarriers ($\{9k, k \in \{0,1,\ldots,7\}\}$) and 10 independent channel realizations (normalized Doppler $f_DT_s=10^{-4}$).}
\label{qtiz_err}
\end{figure}
From Fig. \ref{qtiz_err} it is evident that the reservoir computing based predictive quantizer is able to reduce the quantization error to below what was obtained from the existing temporal correlation based predictive quantizer in \cite{6891198}. In addition, the quantization error is much smoother, which can be explained from the fact that the reservoir computing approach optimizes, $\mathcal{O}_{i,t}$ that captures the long term dynamics of the channel and hence, is able to correct for jittery variations.

We simulate the (uncoded QPSK) BER performance for precoders obtained after interpolation ($\tilde{\bU}_{i\neq\{9k\},t}$) and quantization ($\tilde{\bU}_{\{9k\},t}$), for all the 63 subcarriers.
Observe from Fig. \ref{BER} that reservoir computing framework is able to achieve substantial improvements in $\text{E}_\text{b}\text{N}_0$ levels at $\text{BER}\leq 10^{-3}$.
Particularly, for $\text{BER}=10^{-4}$, we observe around $5$ dB improvement.
In addition, substantial improvement is obtained with both the predictive quantization schemes over the independent quantization scheme with Lloyd codebook for precoders \cite{6678348}, using the same number of bits.

For simulating the achievable rate for the point-to-point MIMO system, we quantize the channel matrix singular values ($\Sigma_{i,t}$ obtained from SVD of $\bH_{i,t}$) using a 2 bit vector quantizer ($k$-means), and feed them back to the transmitter for fed back subcarriers ($\{9k, k \in \{0,1,\ldots,7\}\}$).
For the non-fed back subcarriers, they are interpolated by nai\"ve linear interpolation, as in \cite{Gupt1905:Predictive}.
Observe from Fig. \ref{achievrate} that the reservoir computing framework offers significant improvement in achievable rate for normalized Doppler $f_DT_s=10^{-4}$.
Since temporal variations for normalized Doppler $f_DT_s=10^{-2}, 10^{-1}$ are higher, the reservoir computing framework has to learn a faster-varying function, and hence the degradation in performance.
However, it still matches the performance of the predictive quantizer in \cite{6891198} for normalized Doppler $f_DT_s=10^{-2}$ and slightly outperforms slightly for $f_DT_s=10^{-1}$.
\begin{figure}[h]
\centering
\includegraphics[width=0.45\textwidth]{images/BER_res.pdf}
\caption{BER results after obtaining non fed back $\tilde{\bU}_{i\neq\{9k\},t}$ from $\tilde{\bU}_{\{9k\},t}$ via Cayley interpolation \cite{Gupt1905:Predictive}, averaged over 10 independent channel realizations, 100 channel evolutions each (normalized Doppler $f_DT_s=10^{-4}$).}
\label{BER}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.45\textwidth]{images/hopRate.pdf}
\caption{Achievable rate results for the two predictive quantization methods considered. The results are presented as a percentage of the ideal achievable rate obtained when the transmitter has the accurate (unquantized) and non-interpolated estimates of $\bU_{i,t}$ for each subcarrier and each time instant. The results have been averaged over 10 independent channel realizations, 100 channel evolutions each, for each normalized Doppler value ($f_DT_s$).}
\label{achievrate}
\end{figure}
...
\label{section5}
\vspace{-3pt}
In this paper, we have presented a reservoir computing framework that exploits temporal correlations for predictive quantization of SVD based precoders.
The simulations reveal that reduced quantization error reduces significantly, and BER improves significantly, and achievable rates are higher in when using the proposed framework.
This motivates further study into the framework as well, and future work  would involve a generalization of the framework to capture frequency correlations as well for predictive quantization, and a detailed analysis of impact on performance caused by choice of different reservoir parameters like $\bA,\text{tanh}(\cdot)$, and number of dimensions of the reservoir state.
The reservoir computing framework can also be used to predict the future precoders at the receiver, so as to mitigate issues caused by delay in feedback, on the lines of \cite{chang2011adaptive}, which forms another interesting path for future enhancements.
\vspace{-5pt}
